...
export function ChatPanel({ messages, playerId, canChat, wolfMode, disabledReason, onSend, onClose }: Props) {
  const T = useT();
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div
        className="relative w-80 max-w-[85vw] flex flex-col shadow-2xl"
        style={{ backgroundColor: 'rgba(3,5,7,0.98)', borderLeft: `1px solid ${accent}55` }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 shrink-0" style={{ borderBottom: `1px solid ${accent}33` }}>
              ...
              return (
                <div key={m.id} className={`flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
                  <div className="flex items-center gap-1 px-1">
                    ...
                    <span className="text-[9px] font-cinzel uppercase tracking-wide" style={{ color: nameColor }}>
                      {mine ? T('chat.you') : m.senderName}
                    </span>
                  </div>
                  <div
                    className="max-w-[85%] px-2.5 py-1.5 rounded-lg text-[12px] leading-snug break-words"
                    style={{
                      backgroundColor: isWolf
                        ? 'rgba(80,10,10,0.55)'
                        : mine ? 'rgba(120,53,0,0.55)' : 'rgba(30,26,20,0.85)',
          ...
          {canChat ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-black/50 rounded-lg px-3 py-2 outline-none text-[12px]"

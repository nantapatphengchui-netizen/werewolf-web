      ...
      <div className="flex items-center justify-end gap-2">
        {isHost && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-amber-700/55 bg-black/55 hover:border-amber-600/70 hover:bg-black/65 transition-colors group"
          >
            {/* gear icon */}
            <svg viewBox="0 0 20 20" className="w-3 h-3 text-amber-500 group-hover:text-amber-300 shrink-0 transition-colors" fill="currentColor">
              <path fillRule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" />
            </svg>
            <span className="text-[9px] font-cinzel uppercase tracking-widest text-amber-500 group-hover:text-amber-300 transition-colors">
              Host Tools
            </span>
            {isLocked && (
              <span className="text-[7px] text-amber-400 font-cinzel uppercase tracking-wider border border-amber-600/55 rounded px-1">
                Locked
              </span>
            )}
          </button>
        ...
        {TEST_BOTS_ENABLED && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-stone-600/55 bg-black/55 hover:border-stone-500/70 hover:bg-black/65 transition-colors group"
          >
            {/* users/bots icon */}
            <svg viewBox="0 0 20 20" className="w-3 h-3 text-stone-400 group-hover:text-stone-200 shrink-0 transition-colors" fill="currentColor">
              <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z" />
            </svg>
            <span className="text-[9px] font-cinzel uppercase tracking-widest text-stone-400 group-hover:text-stone-200 transition-colors">
              Dev Bots
            </span>
          </button>
        )}
